...
output_folder = './converted_files3/'  # 변환된 파일을 저장할 폴더
...
for filename in os.listdir(folder_path):
    if filename.endswith('.txt'):  # .txt 파일만 처리
        ...
        df['Chapter'] = df.apply(lambda row: f"{row['Book']} {row['Chapter']}장 {row['Verse']}절", axis=1)
        df = df[['Chapter', 'Content']]  # 필요한 열만 유지
